...
[Gianluca will rewrite this paragraph before submitting] instead of implementing in second quantization only the J operator we implement also the transition operators, the ones that can be written as a one-body operator, we would be able also to describe expecation value of transition between different states (example between an excited state and the ground state)... as in NushellX there is a program that calculates these 1 body operators and then the transition probabilities.
...
In this report we have seen how useful it is to have good routines for bench marking results during the development of a shell model code. We have extensively compared analytic results to our code before implementing new features. In Subsection (\ref{subsec:unit}) we have seen that a lot of time can be saved if unit tests and good debugging routines are implemented early. 

The pairing model (Section \ref{sec:pair}) proved itself as a very useful stepping stone to a more advanced code. NuShellX (Section \ref{sec: NushellX}) provided us with more advanced results to benchmark our code at a later stage. In Subsection (\ref{subsec: results}) we see that the results of our code reproduce calculations done with NuShellX well and with good precision.

There are some major issues with our code, mainly the time spent running the code. NuShellX is a very flexible tool for theoretical calculations that can be done surprisingly fast on a regular laptop.  In Section \ref{sec:discussion} provide a few suggestions to make our code more flexible, but we can conclude that the major issue of our code is the execution time. This problem may be solved by implementing a bit representation and a better diagonalization routine for finding the eigenvalues of the Hamiltonian matrix. 
