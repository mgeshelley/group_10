...
We can repeat an analogue procedure starting from a general $\ket{\Phi_k}$ where $k$ can be different from 0, and we will derive similar expressions for the Hamiltonian matrix elements, meaning that for each $\ket{\Phi_k}$ of the basis of Slater determinants there are only 'limited' $\bra{\Phi_l}$ that can interact with it through $\hat H$. 
