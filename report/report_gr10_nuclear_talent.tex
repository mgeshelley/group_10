...
The code is organized into blocks of code stored in separate files that performs different tasks. There is a \texttt{main.py} program that runs the code and calls the functions in the other files. The structure of the \texttt{main.py} program is as follows:
...
The function that asks the user for input is implemented in such a way that the user is not allowed to provide unphysical input values. If invalid input is given the program provides an error message and asks the user to provide the input within the allowed type or interval. 
