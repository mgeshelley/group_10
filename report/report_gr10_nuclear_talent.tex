...
The atomic nucleus is an interesting playground to study three of the four forces in nature: strong, electromagnetic and weak interaction. We focus our attention on low-energy scale, so, despite the fundamental particles that form nuclei are quarks and gluons, the degrees of freedom we explore are protons and neutrons. One of the aim of nuclear physics is to describe the properties of all the nuclei that seat on the nuclear chart and also make prediction to the limits of possible bound systems (driplines). With properties we indicate bulk observables like binding energy (mass), radius, electromagnetic moments, spin as well as life-time, excitation energies, deformation, electromagnetic transitions, decays.
...
The strong force is the one that keep together nucleons in a bound system, but this interaction is not well mathematically formalized so comparing theoretical results of effective interaction with the available experimental data help us to understand what are the different terms that play role in the interaction models.
...
\end{figure}
The nuclear chart can be described by three different theoretical model, as in Figure \ref{fig: nuclear_chart}.
Light nuclei, $A \lesssim 20$ is the battlefield of ab-initio method where interaction derived by first principles (close to quantum chromodinamics) are applied. Medium mass nuclei, $ 16 \lesssim A \lesssim 100$ can be studied by Configuration Interaction theory, known as shell-model, limited by the truncation of the model space. Heavy nuclei are usually described by Energy Density Functionals, that are less computer power consuming with the price that they can calculate mainly ground state properties.

In this report we consider the Configuration Interaction method. We start to study a basic pairing model to practise with the second quantization formalism and the building block of shell-model, earning analytical results that we implement in a Python code to be used as benchmark.

We learn how to use the state-of-art Configuration Interaction code NushellX and performed calculation of the Oxygen isotopic chain in the $sd$-model space.

We extend our Python code to describe, through full Configuration Interaction method, neutrons in $sd$-model space in order to be able to study a simple isotopic chain as $^{17}$O-$^{28}$O. This development allows us to obtain identical energy levels as the ones calculated with NuShellX, that benchmarks our code beyond the pairing model. 
...
The NuShellX@MSU code provides the model spaces (\texttt{*.sp}) and hamiltonians (\texttt{*.int}) found in the \texttt{sps} folder. The file \texttt{label.dat} contains a list of available model space and hamiltonian combinations. For a more detailed description of how to run the NuShellX code see Ref.\ref{help_file}.
