...
It has the dimension of few fm (10$^{-15}$m) so that the quantum mechanics regulates its structure. 
We are interested in describe low-energy scale that determine the nuclear configuration, so, despite the fundamental particles that form a nucleus are quarks and gluons, the degrees of freedom we explore are protons and neutrons. The number of nucleons can be between one and a couple of hundreds, then many-body treatment is required.\\ 
This sub-atomic system is one of the most attractive laboratory to study physics because in it as well as three different natural forces play an active role: strong, electromagnetic and weak interaction. 
The Coulomb force among protons and the electromagnetic transitions are manifestations of the electromagnetic force.
Beta decays are example of weak interactions.
The strong force is the one that keep together nucleons in a bound state, but this interaction is not yet fully understood (there is not a complete equation for it), then comparing theoretical results of effective interactions with the available experimental data help us to investigate the importance of the different terms in the interaction models.

One of the aim of nuclear physics is to explain the properties of all the observed  nuclei and also make prediction to the limits of possible bound systems (driplines). With properties we mean observables like binding energy (mass), spin, radius, deformation, electromagnetic moments as well as life-time, excitation energies, electromagnetic transitions, decays and particle emissions...
...
\caption{In this nuclear chart the known nuclei are plot according to the neutron number $N$ (x-axis) and proton number $Z$ (y-axis). The domain of Ab-initio, Configuration Interaction and Energy Density Functional is pictured. The nuclei represented in black are stable ones.}
\label{fig: nuclear_chart}
\end{figure}
The nuclear chart can be described by three different theoretical model, as shown in Figure \ref{fig: nuclear_chart}.
Light nuclei, $A \lesssim 20$ is the playground of Ab-initio methods where interaction derived by first principles (close to quantum chromodinamics) are applied and all the nucleons are taken into account. Medium mass nuclei, $ 16 \lesssim A \lesssim 132$ can be studied by Configuration Interaction theory, known as shell-model, in which the truncation of the model space is the key to study a large system but with a doable amount of information (computing resources). Heavy nuclei are usually described by Energy Density Functionals, that reduce the volume of information needed with the price that mainly ground state properties can be calculated.

% ANTICIPATION OF THE CONTENTS
In this report we consider the Configuration Interaction method. In Sec.\ref{sec:pair} we start to study a basic pairing model to practise with the second quantization formalism and the building block of shell-model, earning analytical results that we implement in a Python code to be used as benchmark.

In Sec.\ref{sec: NushellX} we learn how to use state-of-the-art Configuration Interaction code NushellX and performed calculations of the Oxygen isotopic chain in the $sd$-model space.

We extend our Python code in Sec.\ref{sec:ourcode}to describe, through full Configuration Interaction method, neutrons in $sd$-model space in order to be able to study a simple isotopic chain as $^{17}$O-$^{28}$O. This development allows us to obtain identical energy levels as the ones calculated with NuShellX, that benchmarks our code beyond the pairing model.
...
Among the extended functionalities (called by the option \texttt{den}) that make NushellX a powerfull program to study atomic nuclei, there are the possibilities to calculate electromagnetic transitions, neutron and proton emission, beta decay as well as Hartree-Fock calculations in Energy Density Functional style.

We describe in this report only the NuShellX calculations we used to benchmark our code. 
We run NuShellX to calculated the excitation energies of Oxygen isotopes, from $^{17}$O to $^{28}$O included, using the $usdb$ effective interaction optimized for the 1s0d shells. In this code suite it is also possible to compare the theoretical results with the experimental available values and the agreement is good, as in Figure \ref{expnushellx}.
...
\end{array}$
\end{center}
\caption{ Example of calculation done with NushellX for $^{18}$O and $^{20}$O and the relative experimental values. The agreement is good especially for the lowest excited states.}
\label{expnushellx}
\end{figure}
...
\end{equation}
where $\langle p q\vert \hat v \vert rs \rangle_{AS}$ are the two-body antisymmetrized matrix elements for the $\hat v$ interaction. We read them from the file \texttt{sd\_mscheme.int} (point \ref{list: tbme} in the 'structure list', right?) adapted from Ref.\ref{file_link} for the interaction $usdb$ optimized in the $sd$-model space.
...
In our code we have implemented an unit test for the Hamiltonian of the pairing model (point \ref{list: unit_test}). If the Hamiltonian built by our routine is the same as the analytic one derived in Sec.\ref{sec:pair}, matrix element per matrix element, the code moves to perform the diagonalization. If the numerical and analytic Hamiltonian do not match, the code will be exited with an error message. See Code \ref{code: unit_test}.
...
%[is there other ways we tested the code? or crosschecked it?] Gianluca: for me it is enough


%------------------------------------------------

% This could be a subsection of the 'Building' chapter
\subsection{Results}

\paragraph{Benchmark results from NuShellX.} 
The goal of our project was to create a basic version of a shell model code and use the NushellX code to test it when going beyond the pairing model. 

We can make a comparison between our code and NushellX with $usdb$ interaction for the Oxygen isotopes, where varying the number of valence neutrons in our code correspond to varying the number of neutrons on top of the $^{16}$O core. In Figure \ref{fig: isotopes} we can see that the values are identical. This plot is interesting because it show that increasing the number of neutron the system become more bound (g.s. energy decrease) until valence neutron = 8 ($^{24}$O) and then increases sligthly. It means that the neutron separation energy $S_n$ becomes negative and the system is not anymore bound. 
...
%\subsection{Further considerations and possible improvements}
